    ...
    public function valida(Request $request)
    {
        try {
            ...
            select
                ...
                string_agg(distinct u_s.id_setor::text, ',') as setor
            from usuario u
        ...
        ", [$cpf]);
            ...
            $setores = explode(',', $user->setor);

            $perfilRotas = DB::table('tp_rotas_perfil')
                ->whereIn('id_perfil', $perfis)
                ->orderBy('id_rotas')
                ->pluck('id_rotas')
                ->toArray();

            $setorRotas = DB::table('rotas_setor')
                ->whereIn('id_setor', $setores)
                ->orderBy('id_rotas')
                ->pluck('id_rotas')
                ->toArray();

            $rotasAutorizadas = array_intersect($perfilRotas, $setorRotas);
