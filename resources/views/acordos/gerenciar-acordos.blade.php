                        ...
                        <table class="table table-striped table-bordered border-secondary table-hover align-middle">
                            ...
                            <tbody style="font-size: 15px; color:#000000;">
                                @foreach ($acordos as $acordo)
                                    <tr>
                                        <td class="text-center">{{ $acordo->nome }}</td>{{-- tipo de acordo --}}
                                        <td class="text-center">
                                            {{ \Carbon\Carbon::parse($acordo->data_inicio)->format('d/m/Y') }}</td>
                                        {{-- data de inicio --}}
                                        <td>{{ $acordo->valido }}</td>
                                        <td>{{ \Carbon\Carbon::parse($acordo->data_fim)->format('d/m/Y') }}</td>

                                        <td class="text-center">{{ $acordo->observacao }}</td>
                                        <td class="text-center"><a href="{{ asset("$acordo->caminho") }}"
                                                class="btn  btn-outline-secondary" data-tt="tooltip" data-placement="top"
                                                title="Visualizar">
                                        ...
                                        <center>
                                            <td>
                                                ...
                                                <button class="btn btn-outline-danger" {{-- Botão que aciona o modal  --}}
                                                    data-bs-toggle="modal" data-bs-target="#A{{ $acordo->id }}"
                                                    data-tt="tooltip" data-placement="top" title="Excluir">
                                                    <i class="bi bi-trash"></i></button>
                                                </td>
                                                <div class="modal fade"{{--  Modal  --}} id="A{{ $acordo->id }}"
